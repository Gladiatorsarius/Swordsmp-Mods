...
After the plan is finalized, run five review subagents to critique the plan:
1. **The Security Specialist:** Focus on vulnerabilities, data privacy, and permission leaks.
2. **The Performance Engineer:** Look for bottlenecks, unnecessary complexity, or resource-heavy operations.
3. **The QA Lead:** Identify edge cases, missing error handling, and "happy path" assumptions.
4. **The Maintainability Advocate:** Check for technical debt, naming consistency, and adherence to existing patterns.
5. **The Integration Expert:** Ensure the plan accounts for side effects on other modules and dependencies. (See <attachments> above for file contents.)

After collecting the review subagents' outputs, update the plan to incorporate their feedback and address identified issues. Iterate by revising the plan, then re-running the review subagents as needed until no reviewer raises critical concerns.
</workflow>
