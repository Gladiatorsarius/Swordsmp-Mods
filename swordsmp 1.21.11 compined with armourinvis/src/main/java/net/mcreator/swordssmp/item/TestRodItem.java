...
public class TestRodItem extends FishingRodItem {
	public TestRodItem(Item.Properties properties) {
		super(properties.durability(100).repairable(TagKey.create(Registries.ITEM, Identifier.parse("swordssmp:test_rod_repair_items"))).enchantable(2));
	}
			...
			if (world instanceof ServerLevel serverLevel) {
				...
				Projectile.spawnProjectile(new FishingHook(entity, world, k, j) {
					...
					protected boolean shouldStopFishing(Player entity) {
						if (entity.isRemoved() || !entity.isAlive() || !entity.getMainHandItem().is(SwordssmpModItems.TEST_ROD) && !entity.getOffhandItem().is(SwordssmpModItems.TEST_ROD) && this.distanceToSqr(entity) > 1024.0) {
							this.discard();
							return true;
						}
						return false;
